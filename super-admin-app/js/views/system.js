      ...
      btnDebug.addEventListener('click', async function() {
        ...
        try {
          ...
          if (userDoc.exists) {
            ...
            lines.push('');
            lines.push('── Entitlement State (v2) ──');
            lines.push('plan: ' + (data.plan || 'free'));
            lines.push('planType: ' + (data.planType || 'null'));
            lines.push('planSource: ' + (data.planSource || 'null'));
            lines.push('isTrial: ' + (data.isTrial === true ? '✅ YES' : '❌ NO'));
            lines.push('');
            lines.push('── Expiration Details ──');
            var planExpiryMs = AdminUtils.toMillis(data.planExpiry);
            lines.push('planExpiry: ' + (planExpiryMs ? AdminUtils.formatDateTime(data.planExpiry) + (planExpiryMs > now ? ' (ACTIVE)' : ' (EXPIRED)') : 'null'));
            ...
            var planActive = data.plan === 'premium' && (!planExpiryMs || planExpiryMs > now);
            var activeSource = 'FREE';
            if (planActive && data.isTrial) activeSource = 'PREMIUM via TRIAL (active)';
            else if (planActive) activeSource = 'PREMIUM (active)';
            else if (data.plan === 'premium') activeSource = 'PREMIUM (EXPIRED — pending self-heal)';
          ...
          if (coachingDoc.exists) {
            ...
            cLines.push('Students: ' + (cData.studentCount != null ? cData.studentCount : (cData.studentsCount || 0)));
