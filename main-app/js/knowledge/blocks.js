 ...
 *
 * The reusable component layer: BlockRenderers[type](block) → DOM. A new content kind (video, flashcard, diagram…)
 * is a new entry here — never a rewrite. Tables render into the EXISTING `.math-table` markup and formulas into the
 * EXISTING `.formula-block` markup, so QuantReflex's identity (and the loved tables) is preserved exactly; the
 * richer blocks add `.kx-*` classes styled in css/style.css. Browser-only (DOM). No AI surfaces.
 */
var BlockRenderers = (function () {
  ...
  var R = {
    ...
    related: function (b) {
      ...
      (b.ids || []).forEach(function (id) {
        var a = _el('a', 'kx-chip', _esc(id.replace(/-/g, ' ')));
  ...
  /** Section title labels for the in-page section navigation (Phase 2 uses these). */
  var SECTION_LABELS = {
