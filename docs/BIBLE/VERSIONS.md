...
|---|---|---|
| **Bible Version** | 2.34 | The documentation set as a whole (these `/docs/BIBLE/` files). |
| **Architecture Version** | 2.20 | App topology, service boundaries, data-flow contracts. |
| **Firestore Version** | 2.14 | Collection/field/path schema + indexes. |
...
|---|---|---|---|---|---|---|
| 2026-06-14 | 2.34 | 2.20 | 2.14 | 2.12 | 2.3 | **QuanAI live-context + unbroken Explain flow (ADR-045):** the one AI brain was fed stale/thin context. **Fixes:** `studentContext` aiContext cache 6h→90s (+`force` on Coach/Insights open) and now reads the live `today` goldmine + last-session pacing; cold-start computed live from today OR lifetime → no more "complete the warm-up / need 20 questions" after a 26-question day; Coach/Insights coach instead of gate. **Explain** follow-ups anchored to the exact question via a new `explain.followup` prompt (client carries question+lastExplanation) → no Trapezium→Rectangle drift; `preferredDepth` honored. **In-place micro-drill**: new `drill` chip runs 5 adaptive questions inside the modal and returns to the conversation — "Drill this" never navigates. Bug sweep: prompt-version metadata, non-empty deep-link category. No model change (gpt-4o-mini); no schema/rules/index change. Validated by a 19-scenario harness + 12 logic checks. SW v105→v106. Arch 2.19→2.20, Bible 2.33→2.34. |
