...
| TEST-1 | Automated test coverage | Quality | Med | **Partial** — `main-app` `npm test` runs 7 deterministic validators (knowledge-base 3654, planner-engine 238, planner-brain 97, intelligence-consistency 79, notifications 18, ai-cost 12, duel-archive 45 ≈ **4,143 assertions**), plus `scripts/mock-engine.check.js` (100) and `scripts/duel-sim.js` (needs `firebase-admin`). **Closed 2026-08-12:** the payment/entitlement critical-path gap is covered — `payment-parity`, `payment-refund`, `payment-facade`, `refund-policy`, `refund-workflow`, `entitlement-core`, `entitlement-invariants`, `entitlement-ledger`, `entitlement-parity` and `platform` all run in `npm test`, and `mock-engine.check` is wired. `npm test` now chains **56** suites. |
...
- **Vercel Free (Hobby) cap = 12 Serverless Functions/project** (ADR-017). Post-consolidation counts:
  main-app **10**, super-admin **10**, coaching **5** *(re-counted 2026-08-12; this line read 8/8/5 and
  had been two behind since the payment work — a stale budget is dangerous because the cap is what
  decides whether a feature can ship at all)*. New features must fit an existing domain API (no new
