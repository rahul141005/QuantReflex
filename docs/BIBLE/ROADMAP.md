...
| DEBT-3 | `stats.lastActiveDate` is a non-sortable `toDateString` | Cleanup | Medium | Written as `new Date().toDateString()` (`main-app/js/progress.js`), so Firestore `orderBy('stats.lastActiveDate')` sorts **lexicographically, not chronologically** (the coaching roster's server-side "Recent" order + page boundaries are wrong across month/year). Mitigated for normal rosters (ADR-028 cursor fix makes all students reachable + the client re-sorts each loaded page by real date). Proper fix: persist a sortable `stats.lastActiveMs` (epoch) alongside it in `progress.js`, `orderBy` + cursor on that, add a `users(coachingId, stats.lastActiveMs)` index, backfill, and switch the super-admin sweep cron's `lastActiveDate < cutoff` range query to it. Also violates `shared/schemas/user-schema.json` (`format: date-time`). |
