...
- **Context.** A live review of the shipped app (7 screenshots) found that the multi-wave UI overhaul —
  specifically commit `430cb9b` "FW-W4" — had regressed the *presentation* of Home and Practice while the
  underlying architecture (design tokens, primitives, the `qr-ico` icon system, `QROverlay`, light-default, the
  runtime Quick-Links registry) remained sound. The directive was explicit: **stabilize first** — restore what
  regressed, keep the architecture, don't chase novelty. Two independent git-archaeology passes pinned every
  regression to an exact cause and produced the pre-regression markup/CSS as the restore reference. The fixes are
  small, surgical, and architecture-preserving.
- **Decision.** Recover the presentation layer to the pre-FW-W4 quality bar on Home + Practice without reverting
  any architectural gain. Four surgical changes, each root-caused:
  - **Practice — faithful restore (UIR-A).** FW-W4 collapsed the Advanced group from a uniform card wall into thin
    single-line disclosure rows; `.mode-card--row p` (`white-space:nowrap; text-overflow:ellipsis`) was what
    truncated the detail text ("10 ques…", "Multi…"). The Quick-Start cards never actually grew — they only looked
    oversized by contrast to the collapsed rows. Restore = drop `practice-mode-group--rows` + the five
    `mode-card--row` classes (inner markup and every `data-mode` hook byte-identical → **zero JS change**) and
    delete the now-dead `.practice-mode-group--rows` / `.mode-card--row` CSS block (incl. the `::after` chevron and
    the truncating `p` rule). Result: one uniform `.mode-card` wall (Quick Start · Subject Sets · Advanced), no
    truncation, calm rhythm — the "stronger screen" recovered.
  - **Home Explore — balanced bento (UIR-B).** `.home-bento-grid` is a hard 2-col grid; Math Duel spans full width;
    FW-W4 *added* a third `.home-twin-card` (Quick Reference), and an odd 3rd twin orphaned into the last row beside
    an empty cell — the large void in the screenshot. Restore = remove the orphan Quick Reference tile (markup +
    its click/keydown wiring in `home-view.js`) → the balanced pre-FW-W4 Explore returns (Duel full-width, then
    AI Coach | Study Planner twins). Quick Reference remains reachable via its Quick-Study chip and the Learn tab,
    so no destination is lost.
  - **Quick Study — monochrome chips (UIR-C).** `_qlChipInner` preferred a monochrome `ico` mask but fell back to a
    raw color `emoji`; reference/learn registry entries carry only `emoji` (🍕 frac-pct, gray ✖ tables), so the
    default strip mixed color emoji with one clean icon — the unprofessional look. Fix (render layer, registry
    kept): every chip and picker row now resolves a line-icon via a small category-aware helper (reference →
    `book-open`, learn → `graduation`, practice → `target`, features → `layers`), so the strip is uniformly
    monochrome accent-tinted `qr-ico` masks. The v2 registry / `.qs-chip` / picker / pencil-edit affordance are
    untouched (better foundation — scales, localizes, tokenized).
  - **Chrome-emoji cleanup (UIR-D).** Stripped the decorative ✨ from the visible Explore CTA/label strings
    (`home.unlockAiCoach/createStudyPlan`, `ai.unlockCoach/talkToCoach/openPlanner/wizCreatePlan`, `paywall.ctx_*`)
    across en/hi/mr — values only, keys unchanged (i18n parity holds). Consistent with the Wave-B chrome-emoji policy.
- **Consequences.** Home reads stronger than both the original and the interrupted redesign; Practice is faithfully
  restored to its uniform card wall with no truncation. The design system stays token-enforced — the restore
  *removes* CSS and reuses existing classes/tokens, so design-lint ceilings hold by construction (no raw literals
  reintroduced). Out of scope and deliberately untouched: Home hero/stats/Start-Training/Today's-Goal, the AI Coach
  and Study Planner cards themselves (already the preferred version), Word Problems (intentionally removed
  end-to-end earlier), and every other screen. This is a presentation recovery, not a redesign — architecture is
  preserved wholesale. **Verified:** npm test 14,340/0 (−2 vs 14,342 = the two removed Quick-Reference-tile
  data-i18n nodes, 0 failed), payment-parity 25/0, design-lint 10/10; live Playwright screenshots of Home +
  Practice across light/dark × phone/tablet confirm the restore. SW v246→v247.
...
- **Phase-1 follow-up (credit accounting).** `api/ai.js` `_explain` *returns* (not throws) a 400 when question/answer
  are missing, so the dispatch catch's `refundFreeExplain` never fired — but the free credit was already consumed in
  the gate. Now the 400 branch also refunds when `req.freeExplain` was granted, so the refund covers **every** exit
  that spent a credit without delivering content. Narrow (the client always sends both) but airtight.
- **DI charts (`di-charts.js` + `style.css`).** The renderer is pure (spec→SVG string) and dual-exported; the
  check counts `<rect>`/`<path>`/`<polyline>` exactly, so all new axis/grid marks use `<line>`/`<text>` only, and
  `describe()`/aria (the AI-grounding + a11y text) are untouched.
  - **VIS-1 — value scale + gridlines.** `_niceMax` already computed a clean ceiling but never drew it; added
    horizontal gridlines + a numeric y-tick scale to bar/line/grouped/stacked, and vertical gridlines + a bottom
    x-tick scale to the horizontal bar. Recessive hairline styling.
  - **VIS-2 — axis titles.** `xLabel`/`yLabel` reached the spec (from `di-engine.js`) but no renderer drew them; now
    a rotated y-title + a centered x-title render when present (pad grows to fit).
  - **VIS-3 + VIS-4 — one CSS-variable palette.** Replaced per-datum inline `fill="<hex>"` (the rainbow) with
    class-based fills painting via `currentColor` from `--di-series-0..5`. Single-series bar/hbar now use ONE colour
    (VIS-3); multi-series/pie colour by fixed slot order (never cycled). The palette lives once in CSS: brand hues on
    light (CVD worst-adjacent ΔE 21.4; contrast relief via the on-chart value labels) and lightened steps on dark
    (≥3:1 on the `#1e293b` card), so `body.dark-mode` retunes automatically and the renderer stays theme-unaware
    (VIS-4). Palette CVD-validated with the dataviz validator.
- **LR figures (`style.css` LR block + `lr-figures.js` row cap).** `lr-figures.check` asserts class names / counts /
  viewBox / transforms — not stroke widths, px caps, colours, or the badge — so this polish is test-safe;
  `describe()` is spec-driven (unaffected).
  - **VIS-5 — stroke normalization.** Scattered literal widths (1→6) → two CSS tokens: `--lr-stroke` (primary
    outline) + `--lr-stroke-thin` (interior). grid3 renders in a 300×300 viewBox, so its strokes use
    `calc(token*3)` to match the 100-unit figures at display size.
  - **VIS-6 — bigger figures.** Caps raised (prompt 130→156px, option 96→112px, grids proportional, `row` inline
    cap 84→96px/item) for at-a-glance readability.
  - **VIS-7 — badge overlap.** The A/B/C/D chip is now an **opaque** plate (solid bg + hairline + shadow, light+dark)
    so a wide `row` option figure can never render under it.
- **REP-1 — super-admin renders reported charts/figures.** The report detail (`super-admin-app/js/views/reports.js`
  `_tabQuestion`) only dumped raw JSON. The chart/figure specs are **already stored** (`report-context.js` →
  `report-schema.js` → `report.js` persist `chart`/`figure`/`optionFigures`), so this is a pure render change — **no
  main-app capture change**. Reused the **ADR-099/ADR-102 cross-root pattern**: promoted the two pure renderers to
  canonical **`shared/ui/{di-charts,lr-figures}.js`**, with byte-identical app-local copies in main-app and
  super-admin regenerated by **`scripts/sync-visual-renderers.js`** and guarded by
  **`main-app/scripts/visual-renderers.check.js`** (wired into `npm test`). Super-admin gains the two `<script>`
  includes + SW `ASSETS` entries + `APP_VERSION` v13→**v14** (+ `QR_ADMIN_APP_VERSION` lockstep), and the
  chart/figure CSS was ported into `admin-style.css` (main-app `body.dark-mode` → super-admin
  `:root[data-theme="dark"]`, with a keep-in-sync header). `_tabQuestion` now renders `DICharts.render(q.chart)` /
  `LRFigures.render(q.figure)` (+ `optionFigures`) above the stem, **guarding** a missing renderer or null spec (a
  48 KB byte-cap can strip specs; Learn reports carry none) → the Raw-JSON `<details>` remains the fallback.
- **Consequences.** DI charts are readable at exam grade (scale/gridlines/titles, one honest series colour, dark-
  tuned); LR figures are consistent and larger with a legible badge; super-admins can finally see the visual a
  student reported. No Firestore/rules/schema change; main-app rides unreleased `v222` (no re-bump). Deferred (Low):
  DI hover/tooltip layer and figure texture channel — not required for the static exam-review context.
