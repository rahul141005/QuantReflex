...
Replaces the one-shot "Mission" with a deterministic planning engine the LLM only narrates (§0 doctrine). No model
change (still gpt-4o-mini). Bible 2.34→2.35, Arch 2.20→2.21, Firestore 2.14→2.15. SW v106→v107.
...
- **Brain/API** — `aiBrain.js` plannerGet/Setup/Toggle/RegenBlock over `aiPlanner/{uid}` v2; `aiPrompts.js`
  ...
  sanitization. Coach plan-note now reads `aiPlanner` (falls back to legacy `aiMissions`).
...
- **Legacy** — `aiMissions` + `action=mission` + `plan.generate` kept dormant for back-compat (no UI opens them).
