...
| `settings` | map | defaults | client | theme, sound, vibration, difficulty, dailyGoal, etc. **ADR-090:** `targetExam` (QR_SYLLABUS exam id, canonical target-exam identity — written via `TargetExam.set`, mirrored to local `qr_active_exam`) + `targetTier`. **ADR-091:** `appearance` (`'system'\|'light'\|'dark'`, canonical light/dark mode; absent = lazy legacy migration from `darkMode`, which is kept as the derived resolved boolean). Whole-map sync, no whitelist — no migration needed. |
| `stats` | map | defaults | client | attempts, streaks, categoryStats, mistakes[], responseTimes[], **dailyHistory{}** (see note). `categoryStats` is keyed by the 14 drill categories — **the "subject" lens (ADR-073) is DERIVED from it on read, never stored** (see note). **ADR-107:** the today-counters `todayAttempted`/`todayCorrect` are joined by **`diSetsToday`/`lrSetsToday`** — per-day DI/LR-set quota counters (free users get one of each per day; Premium unlimited). Client-managed exactly like `todayAttempted`: incremented by `progress.js recordSetStarted`, reset in the `loadProgress` day-change block, mirrored via whole-map `syncStats` (no whitelist). The 20/day question cap they help enforce is **firm mid-session** (ADR-107) but remains a client-side gate (PREM-4 conscious trade-off — question generation is free/local). |
...
**Subject layer is DERIVED, not stored (ADR-073, V2 Phase 1, 2026-06-30):** QuantReflex's "subject" lens (Quant → Data Interpretation → generatable Logical Reasoning) sits one notch above the 14 drill categories. It is computed **on read** from `stats.categoryStats` via `data/subjects.js#subjectToCategories()` — there is deliberately **no `subjectStats` field, no per-subject collection, and no migration**. A subject's progress is the roll-up of its member categories' `{attempted, correct}`, so old documents are included automatically and storage stays flat. Any future per-subject math must live in the ONE derivation layer (`data/statMath.js`, ADR-053) so Analytics and the Coach can never disagree. **Do not add a stored `subjectStats`** — that would duplicate the source of truth and require a migration for zero benefit. **Data Interpretation (ADR-074, V2 Phase 2)** and **Logical Reasoning (ADR-075, V2 Phase 3)** confirm the design: DI/LR practice simply writes new `di-*` / `lr-*` keys into the existing `stats.categoryStats` map (e.g. `di-bar`, `lr-syllogism`) — **no new fields, no new collection, no migration**; the `di` and `lr` subjects derive from those keys exactly as `quant` does. (LR's multiple-choice questions are graded entirely client-side via the same `recordAnswer` path; nothing about MCQ changes the schema.) **Phase 4 (ADR-076)** adds only DERIVED reads: `statMath.subjectRollup(stats, subjectCats)` rolls `categoryStats` up per subject on read (for Stats + QuanAI) — still **no `subjectStats`, no new field, no migration.**
...
| **`usage/ai`** | **AI quota + cost — SOURCE OF TRUTH** | admin (register, aiService) | `{wordProblemsUsedLifetime, wordProblemsUsedToday, wordProblemsLastDate, explanationsUsed, freeExplanationsUsed, lastUsageDate, insightsGeneratedDate, gptTokensInput, gptTokensOutput, gptCostUSD, gptCalls, gptThrottleDate, gptThrottleCount}` — the four `gpt*` counters (`increment`-written by `aiService.trackGptCost` on every OpenAI call) are per-user token/cost telemetry (added 2026-06-11, Super Admin Phase 1). `gptThrottleDate` (UTC `YYYY-MM-DD`) + `gptThrottleCount` are the **per-user daily throttle counter** (ADR-022) — written transactionally by `aiService.enforceAiThrottle` only when the parent `users/{uid}.aiThrottle.cap` is set, and reset each UTC day. **`freeExplanationsUsed` is (as of ADR-106, 2026-07-07) the ENFORCED free-tier "Explain" meter**: free accounts get 5 lifetime real QuanAI explanations; `aiService.consumeFreeExplain` increments it **transactionally** (deny at ≥5) in the `api/ai.js` gate (survives a storage clear), and `refundFreeExplain` decrements it on a no-content failure. Limit from `services/freeExplainPolicy.js` (lockstep-checked against `shared/constants/entitlements.js` `AI_EXPLANATION_CREDITS`). **`explanationsUsed` is premium telemetry only** — a lifetime count of explanations generated, `.set(merge)`-written by `aiService.trackExplanationUsage`. *(ADR-103 originally overloaded `explanationsUsed` as the free meter; ADR-106 split them because premium telemetry was starving a lapsed-premium user's free allowance.)* |
