...
**Doc Version:** 1.26 · **Architecture Version:** 2.76 (see [VERSIONS.md](VERSIONS.md))
...
A mental-math / quantitative-aptitude training SaaS for competitive-exam aspirants. The catalog is curated (ADR-067) to **17 exams in 4 user-facing tiers** — **MBA** (CAT, XAT, SNAP, NMAT, CMAT, MAH CET, MAT, ATMA), **Banking** (IBPS PO/Clerk, SBI PO, RBI Assistant), **Foundation**, and **Government** (SSC CGL/CHSL/MTS, RRB NTPC) — the exams where fast, no-calculator calculation drives rank. The app drills **36 categories** (`services/quantTopics.js`) — these constitute the **Quant** subject of a **Speed-Aptitude subject layer** (ADR-073/074, V2): a derived lens above category that lets the product grow along the generative-speed spine **Quant → Data Interpretation → generatable Logical Reasoning** without bolting subjects on later. Subject is **derived on read** from `categoryStats` (no stored rollup, no migration). **Quant, Data Interpretation and Logical Reasoning all ship today** — DI (ADR-074) is a generative subject of 5 chart/table families (`di-*`, `js/di-engine.js`); LR (ADR-075 → **ADR-079**) is now a **29-category hybrid platform** across a Foundation→Core→Advanced→Verbal/Critical→Visual syllabus, served by four engines that all self-register into the same `categoryGenerators` and roll up under subject `lr`: a **generative core** (`js/lr-engine.js` — earned-difficulty archetype pools: coding, a blood-relation kinship solver, direction, series, analogy, odd-one-out, ranking, syllogisms, coded inequalities, calendars, clocks, input-output), a **puzzle SET engine** (`js/lr-set-engine.js` — a constraint generator + brute-force solver guarantees a unique seating/floor arrangement, then 3–6 linked MCQs via the drill set-mode), an **authored-content subsystem** (`data/lr-authored/*` + `js/lr-authored-engine.js` — a schema/validator + curated Critical-Reasoning / Statement / Cause-Effect / Course-of-Action / Decision items with teaching explanations; ADR-079 deliberately relaxes the generative-only moat where exam quality demands authored content), and a **generative visual engine** (`js/ui/lr-figures.js` + `js/lr-visual-engine.js` — DPI-independent SVG mirror/water/dice/cube/figure-series/figure-analogy). All reuse the same drill engine, scoring, analytics, Learn, bookmarking and QuanAI; LR's Syllogisms and every generated/authored/visual answer are verified by independent recompute / model-checking / schema validation in `npm test`. **No Firestore migration.** **Cross-subject intelligence (ADR-076, Phase 4):** `statMath.subjectRollup` derives one per-subject view from `categoryStats` (no storage) that BOTH the Stats "aptitude by subject" breakdown and QuanAI's coaching read — so the platform coaches Quant/DI/LR as one (a percentages gap is named as the cause of slow DI). One-tap **Mixed Aptitude** practice draws a balanced cross-subject sprint. Monetized (v2) as **Free** (20 questions/day) → **Premium** (**₹299/6mo or ₹399/12mo** — includes everything: all training features, the full AI suite, Math Duel, and Timed Mocks). One paid tier; trials are time-limited Premium grants. *(ADR-139 correction: this line read ₹349/₹499, the pre-`b4481a0` prices. Canonical source: `services/paymentService.js` `PLAN_CONFIG`, parity-enforced across four code sites by `payment-parity.check.js`.)*
...
- **Content** `data/knowledge/{categories,numbers,arithmetic,commercial,modern,mensuration}.js` — a 5-category
  ...
  **no scaffolds left** (the `scaffold` status remains a supported state for future categories). A content-quality
  gate in `scripts/learn-content.check.js` enforces gold-standard depth on every published topic (589 assertions).
