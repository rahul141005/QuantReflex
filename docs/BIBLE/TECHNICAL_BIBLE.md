...
| `super-admin-app/` | dev.quantreflex.app | Platform admins | `admin:true` claim | `admin/*` (users [list/details + lifecycle: suspend/restore/archive/purge/reset], entitlements, coachings, questions, payments, ai-usage, ai-budget, inactive-users, duels, notifications, system) + `cron/{daily-snapshot,cleanup-sweep}` (Vercel Cron, `CRON_SECRET`-gated) |
