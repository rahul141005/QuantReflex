...
| `PLAY_PACKAGE_NAME` | ⬜ | **Google Play (ADR-147).** **Not needed in production.** The real Play application id `com.quantreflex.app` is a constant in `services/playBillingService.js`. This variable exists only to point a staging deployment at a different Play application, and for the check suites. Setting it in production would risk addressing the wrong Google app. |
